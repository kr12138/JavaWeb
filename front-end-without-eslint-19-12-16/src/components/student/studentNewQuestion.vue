        ...
        methods: {
            searchCByID() {
                if (this.questioningCID !== undefined) {
                    this.$axios.get(
                        'api/course/get/' + this.questioningCID
                    ).then( response => {
            ...
            searchCByName() {
                if (this.questioningCName !== undefined) {
                    this.$axios.get(
                        'api/course/getByName/' + this.questioningCName
                    ).then( response => {
                        console.log(response)
                        if (response.data.flag === 'true') {
                            cSuccess(this.$toastr, '成功获得课程信息！')
            ...
            searchTByID() {
                if (this.questioningTID !== undefined) {
                    this.$axios.get(
                        'api/teacher/get/' + this.questioningTID
                    ).then( response => {
                        console.log(response)
                        if (response.data.flag === 'true') {
            ...
            searchTByName() {
                if (this.questioningTName !== undefined) {
                    this.$axios.get(
                        'api/teacher/getByName/' + this.questioningTName
                    ).then( response => {
                        console.log(response)
                        if (response.data.flag === 'true') {
                            cSuccess(this.$toastr, '成功获得教师信息！')
