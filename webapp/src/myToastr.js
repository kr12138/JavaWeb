...
export function cToastrInit(toastr, msg) {
    toastr.options = {
        closeButton: false,                      // 是否显示关闭按钮，（提示框右上角关闭按钮）
        debug: false,                            // 是否使用deBug模式
        progressBar: true,                       // 是否显示进度条，（设置关闭的超时时间进度条）
        positionClass: "toast-center",           // 设置显示的位置
        onclick: null,                           // 点击消息框自定义事件
        showDuration: "300",                     // 显示动画的时间
        hideDuration: "1000",                    // 消失的动画时间
        timeOut: "1000",                         // 自动关闭超时时间
...
export function cInfo(toastr, msg, title) {
    cToastrInit(toastr, msg)
    if (title === undefined)
        title = ''
    toastr.info(msg, title)
}
export function cWarning(toastr, msg, title) {
    cToastrInit(toastr, msg)
    if (title == null)
        title = ''
    toastr.warning(msg, title)
}
export function cSuccess(toastr, msg, title) {
    cToastrInit(toastr, msg)
    if (title == null)
        title = ''
    toastr.success(msg, title)
}
export function cError(toastr, msg, title) {
    cToastrInit(toastr, msg)
    if (title == null)
        title = ''
    toastr.error(msg, title)
